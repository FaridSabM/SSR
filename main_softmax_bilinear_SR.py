...
# -*- coding: utf-8 -*-
"""
%% Softmax-Bilinear-SR: Softmax-based Bilinear Self-Representation for Unsupervised Feature Selection
%% Farid Saberi-Movahed
%% Email: f.saberimovahed@kgut.ac.ir; fdsaberi@gmail.com

%% Optimization problem:
%% min_{A,B} ||X - AXB||_F^2 + alpha * ||B||_{2,1}
%%
%% subject to:
%% B = Softmax_row(Z),
%% A = Softmax_column(Y)
...
% alpha    : regularization parameter for the l2,1 norm of B
...
%% Required function:
%% [A, B, X_selected] = softmax_bilinear_SR(X, alpha, lr, max_iter, k, tol, eps)
%%
%% Inputs:
%   X          : (m × n) data matrix
...
%% functions.Bestmap
%% functions.Purity
%% sr_methods.Softmax_Bilinear_SR
"""
...
from sr_methods.Softmax_Bilinear_SR import softmax_bilinear_SR


def run_softmax_bilinear_sr():
    np.random.seed(0)

    NameMethod = 'Softmax-Bilinear-SR'
    ...
    k = 80             # Suggested range {10, 20, ..., 100}
    ...
    # Proposed method
    # -------------------------------------------------------------
    A, B, X_selected = softmax_bilinear_SR(
        X, alpha, lr, max_iter, k, tol=tol, eps=eps
    )
