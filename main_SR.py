...
# -*- coding: utf-8 -*-
"""
%% SR: Self-Representation for Unsupervised Feature Selection
...
% B        : (n × n) self-representation coefficient matrix
...
%% Required function:
%% [B, X_selected] = SR_m(X, alpha, max_iter, k, eps)
%%
%% Inputs:
%   X          : (m × n) data matrix
...
from sr_methods.SR_core import SR_m
...
def run_sr():
    ...
    #   Y : ground-truth labels of shape (m,)
    #
    # If your file has different key names, replace 'X' and 'Y' below.
    ...
    # Initial parameters
    # -------------------------------------------------------------
    max_iter = 10      # Suggested range {5, 10, 20, 30}
    ...
    # Proposed method
    # -------------------------------------------------------------
    B, X_selected = SR_m(X, alpha, max_iter, k, eps=eps)
